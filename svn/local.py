...
_STATUS_ENTRY = collections.namedtuple(
    "_STATUS_ENTRY", ["name", "type_raw_name", "type", "revision",]
)
...
class LocalClient(svn.common.CommonClient):
    def __init__(self, path_, *args, **kwargs):
        ...
        super(LocalClient, self).__init__(path_, svn.constants.LT_PATH, *args, **kwargs)
    ...
    def add(self, rel_path, do_include_parents=False):
        ...
        self.run_command("add", args, wd=self.path)

    def commit(self, message, rel_filepaths=[]):
        args = ["-m", message] + rel_filepaths

        output = self.run_command("commit", args, wd=self.path)
    ...
    def status(self, rel_path=None):
        ...
        raw = self.run_command("status", ["--xml", path], do_combine=True)
