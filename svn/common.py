...
class CommonClient(svn.common_base.CommonBase):
    ...
    def properties(self, rel_path=None):
        ...
        result = self.run_command(
            "proplist", ["--xml", full_url_or_path], do_combine=True
        )
    ...
    def log_default(
        ...
        timestamp_from_phrase = (
            ("{" + timestamp_from_dt.isoformat() + "}") if timestamp_from_dt else ""
        )

        timestamp_to_phrase = (
            ("{" + timestamp_to_dt.isoformat() + "}") if timestamp_to_dt else ""
        )
        ...
        if timestamp_from_phrase or timestamp_to_phrase:
            if not timestamp_from_phrase:
                raise ValueError(
                    "The default log retriever can not take a TO "
        ...
        if revision_from or revision_to:
            if timestamp_from_phrase or timestamp_to_phrase:
                raise ValueError(
                    "The default log retriever can not take both "
            ...
            for entry in self.list(extended=True, rel_path=current_rel_path):
                ...
                if entry["is_directory"] is False or yield_dirs is True:
                    current_rel_path_phrase = (
                        current_rel_path if current_rel_path is not None else ""
                    )
    ...
    def diff_summary(self, old, new, rel_path=None):
        ...
        arguments = [
            "--old",
            "{0}@{1}".format(full_url_or_path, old),
            "--new",
            "{0}@{1}".format(full_url_or_path, new),
            "--summarize",
            "--xml",
        ]

        result = self.run_command("diff", arguments, do_combine=True)
    ...
    def diff(self, old, new, rel_path=None):
        ...
        while True:
            ...
            file_hunk_raw, diff_result = (
                diff_result[:next_index],
                diff_result[next_index:],
            )
    ...
    def _split_file_hunk(self, file_hunk):
        ...
        while True:
            ...
            hunks.append(
                {"lines_phrase": hunk_lines_phrase, "body": "\n".join(hunk_lines),}
            )
